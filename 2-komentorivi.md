...
Tech Infokartin YouTube videossa Guest Additions asennettiin graafisesti, mutta omasta näkymästäni puuttui kohta "Run Software". Tästä syystä pyysin ChatGPT 5.3 -kielimallilta komennot tekemään sama komentorivillä. Suoritin seuraavat komennot ja sain copy-pasten toimimaan. 
'''cd /media/ilona/VBox_GAs_7.2.6'''
'''sudo apt update'''
'''sudo apt install build-essential dkms linux-headers-$(uname -r)'''
'''sudo sh VBoxLinuxAdditions.run'''
'''sudo reboot'''

Opin tästä, että kopiointi ja liittäminen ovat VM:lle lisäominaisuuksia, ja näiden saaminen vaatii erillisten lisäosien asentamista. CD-asema, ISO-levykuva ja optinen asema sekä näiden tarkoitus ei kuitenkaan ihan selvinnyt. Opin kuitenkin hiukan komentorivin käyttöä: hakemistoon siirtyminen (cd /media/ilona/VBox_GAs_7.2.6), pakettilistan päivitys (sudo apt update'), pakettien asennus (sudo apt install build-essential dkms linux-headers-$(uname -r)), asennusskriptin suorittaminen (sudo sh VBoxLinuxAdditions.run) ja järjestelmän uudelleenkäynnistys (sudo reboot). 
